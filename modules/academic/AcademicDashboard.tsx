...
export function AcademicDashboard({ config, module, onUpdate, isEditing = false, date }: AcademicDashboardProps) {
  ...
  const currentTaskCount = useMemo(() => todayEvents.filter((event) => event.type === 'task').length, [todayEvents]);
  const currentExamCount = useMemo(() => todayEvents.filter((event) => event.type === 'exam').length, [todayEvents]);
  ...
  const summaryLabel = selectedDate === getLocalDateString() ? 'Hoy' : `Para ${selectedDate}`;

  if (loading) {
    return <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">Cargando Gestión Universitaria...</div>;
  }
    ...
    <section className="space-y-6">
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          >
            {showConfig ? 'Ocultar materias' : 'Editar materias'}
          </button>
        </div>

        <div className="mt-6 grid gap-4 sm:grid-cols-3">
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-950">
            <p className="text-xs uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">{summaryLabel}</p>
            <p className="mt-2 text-3xl font-semibold text-slate-900 dark:text-white">{currentExamCount}</p>
            <p className="text-sm text-slate-500 dark:text-slate-400">Exámenes registrados</p>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-950">
            <p className="text-xs uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">{summaryLabel}</p>
            <p className="mt-2 text-3xl font-semibold text-slate-900 dark:text-white">{currentTaskCount}</p>
            <p className="text-sm text-slate-500 dark:text-slate-400">Tareas activas</p>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-950">
      ...
      {showConfig && (
        <AcademicConfig subjects={subjects} onSaveSubjects={saveSubjects} onDeleteSubject={deleteSubject} />
      )}

      <AcademicEventForm subjects={subjects} onAddEvent={handleAddEvent} isSaving={isSaving} />
