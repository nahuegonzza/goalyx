...
export default function GoalManager() {
  ...
  const activeGoals = goals.filter((goal) => goal.isActive !== false);
  const booleanGoals = activeGoals.filter(g => g.type === 'BOOLEAN').sort((a, b) => {
    const orderA = typeof a.order === 'number' ? a.order : Number.MAX_SAFE_INTEGER;
    const orderB = typeof b.order === 'number' ? b.order : Number.MAX_SAFE_INTEGER;
    return orderA - orderB;
  });
  const numericGoals = activeGoals.filter(g => g.type === 'NUMERIC').sort((a, b) => {
    const orderA = typeof a.order === 'number' ? a.order : Number.MAX_SAFE_INTEGER;
    const orderB = typeof b.order === 'number' ? b.order : Number.MAX_SAFE_INTEGER;
    return orderA - orderB;
  });
            ...
            {showInactive ? (
              ...
              <>
                {booleanGoals.length > 0 && (
                  <div className="space-y-4">
                    <p className="text-xs uppercase tracking-[0.15em] text-slate-500 dark:text-slate-400 font-semibold mb-4">
                      Hábitos
                    </p>
                    <div className="space-y-4">
